...
class PushPull_grad_norm_track_different_learning_rate(Optimizer):
    def __init__(
            self, 
            model_list: list, 
            lr_list: list, 
        ...
        defaults = dict(lr_list=lr_list)
        super(PushPull_grad_norm_track_different_learning_rate, self).__init__(model_list[0].parameters(), defaults)

    def step(self, closure, lr_list):
        self.lr_list = lr_list  # Update lr if provided
        ...
        with torch.no_grad():
            ...
            new_params_tensor = torch.matmul(self.A, prev_params_tensor) - torch.stack([lr * v for lr, v in zip(self.lr_list, self.v_list)])
